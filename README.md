...
Teste — React + Vite + TypeScript. Autenticação via Keycloak (OIDC).

## Requisitos

- Node.js 24.x (`nvm use`)
- Backend rodando (veja [`chatbot-back`](../chatbot-back))

## Setup

```bash
cp .env.example .env      # já aponta para http://localhost:3000/chatbot/api em dev
npm install
```
...
- `VITE_API_URL` — URL da API (dev: `http://localhost:3000/chatbot/api`).
...
- A aplicação é servida sob um base path (`VITE_BASE_URL`, `/chatbot` por padrão), injetado no bundle na inicialização do container pelo `docker-entrypoint.sh`.
