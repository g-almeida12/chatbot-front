...
| **Nome técnico** | `kebab-case`, sugerido a partir do nome (ex: `loja-do-ze`) | Vira `chatbot` — pastas, pacote npm, client do Keycloak, URL base |
| **Diretório de destino** | Caminho, padrão `./<nome-técnico>-front` | Precisa não existir ou estar vazio |
| **Título** | Texto livre, padrão o nome do projeto | Aba do navegador, sidebar e tela de login (`CIn ChatBot`) |
| **Descrição** | Texto livre, padrão `Painel administrativo do <nome>` | `package.json` e README gerado (`Teste`) |
...
- **`VITE_API_URL`** — URL base da API backend (dev: `http://localhost:3000/chatbot/api/v1`).
...
- **`VITE_BASE_URL`** — base path público da aplicação (`/chatbot` por padrão), lido de `import.meta.env.BASE_URL` em runtime.
