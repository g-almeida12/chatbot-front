...
if [ -n "$list" ]; then
    ...
    if grep -qE "__chatbot_url__|<chatbot_apiurl>|\"<chatbot_keycloak_json>\"" "$list"; then
        sed -i "s|__chatbot_url__|$BASEURL|g" "$list"
        sed -i "s;<chatbot_apiurl>;$API_URL;g" "$list"
        sed -i "s;\"<chatbot_keycloak_json>\";'$KEYCLOAK_PUBLIC_JSON';g" "$list"
        echo "✅ Runtime injection aplicado ao bundle."
    fi
fi

if grep -q "__chatbot_url__" /usr/share/nginx/html/index.html 2>/dev/null; then
    sed -i "s|__chatbot_url__|$BASEURL|g" /usr/share/nginx/html/index.html
fi
